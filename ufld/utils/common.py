...
from utils.dist_utils import is_main_process, DistSummaryWriter
...
def auto_backup_project(auto_backup, to_path):
    # copy project，用来自动拷贝项目做备份的。似乎不太需要。
    if is_main_process() and auto_backup:
        ...
        logger.info('Copying projects to ' + to_path + ' for backup')
        t0 = time.time()
        warning_flag = True
        for f in to_cp_files:
            ...
            if elapsed_time > 5 and warning_flag:
                logger.info(
                    ...
                    '--auto_backup. Or please make you working directory clean, i.e, don\'t place large files like '
                    'dataset, log results under this directory.')
                warning_flag = False
...
def get_summary_writer(work_dir, cfg):
    summary_writer = DistSummaryWriter(work_dir)
